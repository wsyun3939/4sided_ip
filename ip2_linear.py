...
for index in range(NUMBER,NUMBER+100*DEPTH):
    ...
    for i in range(0,WIDTH):
        for j in range(0,DEPTH):
            for t in range(0,NBLOCK-1):
                m.addConstr((1-dir[t])+(1-gp.quicksum(y[i,j_dash,t,t] for j_dash in range(0,j)))-1 <= 1-gp.quicksum(Ux[i,j,k,l,n,t] for k in range(0,WIDTH) for l in range(0,DEPTH) for n in range(t+1,NBLOCK)))
                m.addConstr((1-dir[t]) + gp.quicksum(b[i,j,n,t] for n in range(t+1,NBLOCK)) - 1 <= 1-(gp.quicksum(y[i,j_dash,t,t] for j_dash in range(0,j))-gp.quicksum(Ux[i,j,k,l,n,t] for k in range(0,WIDTH) for l in range(0,DEPTH) for n in range(t+1,NBLOCK))))

    for i in range(0,WIDTH):
        for j in range(0,DEPTH):
            for t in range(0,NBLOCK-1):
                m.addConstr(dir[t] + (1-gp.quicksum(y[i_dash,j,t,t] for i_dash in range(0,i))) - 1 <= 1 - gp.quicksum(Rx[i,j,k,l,n,t] for k in range(0,WIDTH) for l in range(0,DEPTH) for n in range(t+1,NBLOCK)))
                m.addConstr(dir[t] + gp.quicksum(b[i,j,n,t] for n in range(t+1,NBLOCK)) - 1 <= 1 - (gp.quicksum(y[i_dash,j,t,t] for i_dash in range(0,i))-gp.quicksum(Rx[i,j,k,l,n,t] for k in range(0,WIDTH) for l in range(0,DEPTH) for n in range(t+1,NBLOCK))))

    #取り出し方向が上の場合，Rx==0
    for t in range(0,NBLOCK-1):
        m.addConstr(gp.quicksum(Ux[i,j,k,l,n,t] for i in range(0,WIDTH) for j in range(0,DEPTH) for k in range(0,WIDTH) for l in range(0,DEPTH) for n in range(t+1,NBLOCK)) <= 100*(1-dir[t]))

        m.addConstr(gp.quicksum(Rx[i,j,k,l,n,t] for i in range(0,WIDTH) for j in range(0,DEPTH) for k in range(0,WIDTH) for l in range(0,DEPTH) for n in range(t+1,NBLOCK)) <= 100*dir[t])
            ...
            for k in range(0,WIDTH):
                for t in range(0,NBLOCK-1):
                    for l in range(0,DEPTH):
                        ...
                        b_left=1
                        if l > 0:
                            b_low=gp.quicksum(b[k,l-1,n,t] for n in range(t+1,NBLOCK))+gp.quicksum(Ux[i,j_dash,k,l-1,n,t] for j_dash in range(j+1,DEPTH) for n in range(t+1,NBLOCK))+gp.quicksum(Rx[i_dash,j,k,l-1,n,t] for i_dash in range(i+1,WIDTH) for n in range(t+1,NBLOCK))
                        if k > 0:
                            b_left=gp.quicksum(b[k-1,l,n,t] for n in range(t+1,NBLOCK))+gp.quicksum(Ux[i,j_dash,k-1,l,n,t] for j_dash in range(j+1,DEPTH) for n in range(t+1,NBLOCK))+gp.quicksum(Rx[i_dash,j,k-1,l,n,t] for i_dash in range(i+1,WIDTH) for n in range(t+1,NBLOCK))
                        ...
                        x_sum=gp.quicksum(Ux[i,j,k,l,n,t]+Rx[i,j,k,l,n,t] for n in range(t+1,NBLOCK))
    ...
    for n in range(t+1,NBLOCK):
        for i in range(0,WIDTH):
            for j in range(0,DEPTH):
                for k in range(0,WIDTH):
                    for l in range(0,DEPTH):
                        if Ux[i,j,k,l,n,t].x==1:
                            print("上方向(",i,j,")","(",k,l,")",n+1)
                        if Rx[i,j,k,l,n,t].x==1:
                            print("右方向(",i,j,")","(",k,l,")",n+1)
